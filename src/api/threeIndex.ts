...
const URL: string = import.meta.env.VITE_APP_SUB_API_URL2
		...
		this.service.interceptors.response.use(
			(response: AxiosResponse) => {
				...
				if (data.code && data.code !== RequestEnums.SUCCESS) {
					...
					Toast.fail(data.message || data.result)
					return data
				}
				console.log('cc', data)
				return data.result
...
export function threeGet<T>(url: string) {
	return request.get<T>(url)
}

export function threePost<T, P>(url: string, params: P) {
